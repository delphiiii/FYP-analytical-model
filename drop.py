...
class Droplet:
    ...
    def set_Fd(self, rho_flow:float, mu_flow:float, u_flow:float) -> None:
        ...
        if self.Cd:
            ...
            self.Fd = 3*np.pi*mu_flow*self.diameter*v_rel_mag # Stoke's Law for very low Re
...
def optimising_function(e_field_props, flow:ChannelFlow, t_step:float,
...
def visualise(flow:ChannelFlow, delta_t, *, n_phase=2, labels=None, title=None):
    """Solves and visualises given fully defined flow

    Parameters:
    ----------
        flow : ChannelFlow
            Fully defined flow object (with electric field) wish to be optimised
        t_step : float
            Time step for optimiser (s)
        v_max : float
            Maximum allowable voltage between electrodes
        num_phase : int
            Number of evenly spaced phases to solve together
        init_conds : [float]
            Initial electric field conditions for optimiser. Defaults [7.5e4, 100, -2e4]
        parallel : bool
    ...
    solutions, params = solve(flow, t_step=delta_t, num_phase=n_phase)
    ...
    # Plot results
    fig, axis = plt.subplots(figsize=(flow.length*50, flow.height*50))

    for solution in solutions:
        drop_sol = np.array([])
        for phase_solution in solution.values():
            drop_sol = np.append(drop_sol, phase_solution)
        drop_sol = np.reshape(drop_sol, (-1,4))
        sns.lineplot(x=drop_sol[:,0], y=drop_sol[:,1], alpha=0.5, estimator=None, ax=axis)

    plt.ylim([-flow.height/2, flow.height/2])
    plt.xlim([0, flow.length])
    plt.legend(labels)
    plt.title(title)
    return fig, axis
